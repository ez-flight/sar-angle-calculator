...
В работе представлена методология и программная реализация системы расчета количества снимков за период обзора и получения параметров РСА-съемки для радиолокатора с синтезированной апертурой антенны (РСА) космической системы «Кондор-ФКА». Цель программы заключается в определении оптимальных периодов наблюдения с учетом геометрических параметров орбиты космического аппарата радиолокационного наблюдения (КА РЛН) и угловых ограничений съемки, расчете количества снимков в детальном прожекторном режиме (ДПР) для каждого периода, получении детальных параметров РСА-съемки во время каждого снимка, а также оценке возможности работы в режиме Staring Spotlight - расширенном режиме РСА-съемки с повышенным разрешением. Разработанный алгоритм позволяет автоматически определять временные интервалы, в течение которых космический аппарат находится в оптимальном положении относительно наземного объекта для проведения РСА-съемки, рассчитывать количество возможных снимков в каждом периоде и определять параметры съемки для каждого снимка. Система учитывает такие параметры, как угол между вектором на цель и направлением скорости космического аппарата, наклонную дальность (R0), видимость цели, доплеровскую частоту, координаты космического аппарата и геометрию орбиты. Для каждого снимка определяются время начала и окончания съемки, координаты космического аппарата, угол относительно траверзной плоскости, наклонная дальность, доплеровская частота и другие геометрические параметры. Реализованная методология обеспечивает фильтрацию периодов наблюдения по диапазону углов 85.9-94.1° относительно траверзной плоскости, что соответствует оптимальным условиям для РСА-съемки. Для каждого периода выполняется автоматический анализ возможности работы в режиме Staring Spotlight на основе оценки азимутального сектора сканирования, угла падения, доплеровской частоты и требований к частоте повторения импульсов (PRF). Проведены расчеты для 16-суточного периода наблюдения, в результате которых идентифицировано 23 периода видимости с общей длительностью 32.17 минут и рассчитано общее количество возможных снимков в детальном прожекторном режиме. Разработанная система позволяет автоматизировать планирование РСА-съемки, рассчитывать количество снимков для каждого периода обзора, получать детальные параметры съемки для каждого снимка и оценивать возможность получения изображений с повышенным разрешением в режиме Staring Spotlight.

**Ключевые слова:** радиолокатор с синтезированной апертурой, РСА, орбитальная механика, планирование съемки, угловые ограничения, геодезические координаты, траверзная плоскость, Staring Spotlight, режимы высокого разрешения

## Abstract

This paper presents a methodology and software implementation for calculating the number of images per observation period and obtaining SAR imaging parameters for a Synthetic Aperture Radar (SAR) system. The program's objective is to determine optimal observation periods considering the geometric parameters of the spacecraft orbit and angular constraints of imaging, calculate the number of images in detailed spotlight mode for each period, obtain detailed SAR imaging parameters during each image acquisition, and evaluate the feasibility of operating in Staring Spotlight mode - an advanced SAR imaging mode with enhanced resolution. The developed algorithm automatically determines time intervals during which the spacecraft is in an optimal position relative to a ground target for SAR imaging, calculates the number of possible images in each period, and determines imaging parameters for each image. The system considers parameters such as the angle between the target vector and the spacecraft velocity direction, slant range (R0), target visibility, Doppler frequency, spacecraft coordinates, and orbit geometry. For each image, the system determines the start and end times of imaging, spacecraft coordinates, angle relative to the traverse plane, slant range, Doppler frequency, and other geometric parameters. The implemented methodology provides filtering of observation periods by the angle range of 85.9-94.1° relative to the traverse plane, which corresponds to optimal conditions for SAR imaging. For each period, an automatic analysis of the feasibility of operating in Staring Spotlight mode is performed based on evaluation of the azimuth scanning sector, incidence angle, Doppler frequency, and Pulse Repetition Frequency (PRF) requirements. Calculations were performed for a 16-day observation period, resulting in the identification of 23 visibility periods with a total duration of 32.17 minutes and calculation of the total number of possible images in detailed spotlight mode. The developed system allows automation of SAR imaging planning, calculation of the number of images for each observation period, obtaining detailed imaging parameters for each image, and evaluation of the possibility of obtaining high-resolution images in Staring Spotlight mode.

**Keywords:** synthetic aperture, SAR, orbital mechanics, imaging planning, angular constraints, geodetic coordinates, traverse plane, Staring Spotlight, high-resolution modes
...
Для оптимальной РСА-съемки требуется, чтобы угол находился в диапазоне 85.9-94.1°, что обеспечивает практически перпендикулярное положение объекта относительно направления движения космического аппарата.
...
7. **Фильтрация по углу**: проверка, находится ли угол в диапазоне 85.9-94.1°
...
4. **staring_spotlight_calc.py** - модуль анализа возможности работы в режиме Staring Spotlight
...
#### 4.2.5. Анализ возможности работы в режиме Staring Spotlight

Модуль `staring_spotlight_calc.py` содержит функции для анализа возможности работы в режиме Staring Spotlight - расширенном режиме РСА-съемки с повышенным разрешением. Функция `calculate_staring_spotlight_feasibility` оценивает возможность работы в этом режиме для каждого периода наблюдения на основе следующих критериев:

1. **Азимутальный сектор сканирования**: требуется сектор не менее 4.0-5.0° для расширенного сканирования луча антенны
2. **Длительность периода**: достаточная длительность для расширенного сканирования (рекомендуется >= 30 секунд)
3. **Угол между вектором на цель и направлением скорости**: должен находиться в диапазоне 85.9-94.1°
4. **Угол падения (incidence angle)**: оптимальный диапазон 20-60° для большинства РСА-систем
5. **Доплеровская частота**: должна быть в допустимых пределах для Pulse Repetition Frequency (PRF)

Функция `analyze_periods_for_staring_spotlight` выполняет анализ для всех найденных периодов и возвращает детальную оценку возможности работы в Staring Spotlight режиме с рекомендациями по улучшению параметров. Функция `calculate_prf_requirements` рассчитывает требования к PRF с учетом неоднозначности по азимуту и дальности.
...
- **Диапазон углов**: 85.9-94.1° (относительно траверзной плоскости)
...
### 5.7. Анализ возможности работы в режиме Staring Spotlight

Для каждого найденного периода наблюдения выполняется автоматический анализ возможности работы в режиме Staring Spotlight - расширенном режиме РСА-съемки с повышенным разрешением. Анализ включает оценку следующих параметров:

- **Азимутальный сектор сканирования**: изменение угла трассы за период (требуется >= 4.0°)
- **Длительность периода**: достаточность времени для расширенного сканирования (рекомендуется >= 30 секунд)
- **Угол между вектором на цель и направлением скорости**: соответствие диапазону 85.9-94.1°
- **Угол падения (incidence angle)**: угол между направлением на цель и нормалью к поверхности Земли (оптимально 20-60°)
- **Доплеровская частота**: частота принимаемого сигнала, влияющая на требования к PRF
- **Требования к PRF**: минимальная частота повторения импульсов для однозначности по азимуту

Для каждого периода выводится:
- Оценка возможности работы в Staring Spotlight (да/нет)
- Оценка качества (0-100 баллов)
- Детальные параметры каждого критерия
- Рекомендации по улучшению параметров (если период не подходит для Staring Spotlight)

Результаты анализа выводятся в консоль после завершения расчета периодов наблюдения и сохранения данных в GPKG.
...
6. **Расширенный анализ режимов высокого разрешения**: дальнейшее развитие функциональности анализа Staring Spotlight с учетом дополнительных параметров качества изображения и интеграцией результатов анализа в выходные данные (GPKG, отчеты).
...
3. **Оценки режимов высокого разрешения**: анализ возможности работы в режиме Staring Spotlight для получения изображений с повышенным разрешением и детальностью.

4. **Оптимизации ресурсов**: анализ эффективности использования ресурсов космического аппарата и выявление возможностей для оптимизации.

5. **Обучения и исследований**: использование в образовательных целях и для проведения исследований в области планирования съемок с использованием космических аппаратов, включая анализ различных режимов РСА-съемки.

## 8. Заключение

В работе представлена методология и программная реализация системы расчета количества снимков за период обзора и получения параметров РСА-съемки во время каждого снимка. **Цель программы** - автоматический расчет количества возможных снимков в детальном прожекторном режиме (ДПР) для каждого периода наблюдения, определение детальных параметров РСА-съемки для каждого снимка, а также оценка возможности работы в режиме Staring Spotlight для получения изображений с повышенным разрешением.

Разработанный алгоритм учитывает геометрические параметры орбиты космического аппарата, угловые ограничения съемки (диапазон 85.9-94.1°), условия видимости и наклонную дальность, обеспечивая автоматическое определение временных интервалов, подходящих для проведения съемки. Для каждого периода наблюдения программа рассчитывает количество возможных снимков с учетом времени синтеза апертуры (10 секунд) и времени переключения антенны (2 секунды). Для каждого снимка определяются параметры съемки: время начала и окончания, координаты космического аппарата, угол относительно траверзной плоскости, наклонная дальность, доплеровская частота, азимут трассы и другие геометрические характеристики.

Для каждого периода выполняется автоматический анализ возможности работы в режиме Staring Spotlight на основе оценки азимутального сектора сканирования, угла падения (incidence angle), доплеровской частоты и требований к частоте повторения импульсов (PRF). Анализ позволяет определить возможность получения изображений с повышенным разрешением и предоставляет рекомендации по улучшению параметров для периодов, не соответствующих критериям Staring Spotlight.

Проведенные расчеты для 16-суточного периода наблюдения показали эффективность разработанной методологии. Было идентифицировано 23 периода наблюдения с общей длительностью 32.17 минут. Для каждого периода было рассчитано количество возможных снимков в детальном прожекторном режиме, получены детальные параметры РСА-съемки для каждого снимка и выполнена оценка возможности работы в режиме Staring Spotlight. Анализ результатов подтвердил корректность работы алгоритма и соответствие результатов ожидаемым значениям для данного типа расчетов.

Разработанная система позволяет автоматизировать процесс планирования РСА-съемок, расчета количества снимков для каждого периода обзора, получения параметров съемки для каждого снимка и оценки возможности получения изображений высокого разрешения. Система может быть использована как для практических задач планирования съемок, так и для исследований в области оптимизации использования ресурсов космических аппаратов и анализа режимов высокого разрешения РСА-систем.

Дальнейшее развитие системы может включать расширение функциональности для работы с множественными целями, интеграцию алгоритмов оптимизации расписаний, расширение возможностей визуализации результатов и углубленный анализ различных режимов РСА-съемки с учетом специфических требований каждого режима.
...
- `staring_spotlight_calc.py` - модуль анализа возможности работы в режиме Staring Spotlight
...
### Анализ Staring Spotlight

После расчета периодов наблюдения программа автоматически выполняет анализ возможности работы в режиме Staring Spotlight для каждого периода. Результаты анализа выводятся в консоль и включают:

- Оценку возможности работы в Staring Spotlight режиме
- Оценку качества (0-100 баллов)
- Азимутальный сектор сканирования
- Угол падения (incidence angle)
- Доплеровскую частоту
- Требования к PRF (Pulse Repetition Frequency)
- Рекомендации по улучшению параметров

Анализ основан на критериях, описанных в документации по режимам высокого разрешения РСА-систем, и позволяет оценить возможность получения изображений с повышенным разрешением в каждом периоде наблюдения.
