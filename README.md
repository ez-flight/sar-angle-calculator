...
2. **periods_squares** - квадратные рамки для каждого снимка в детальном прожекторном режиме (ДПР) с атрибутами:
   - period_id - идентификатор периода
   ...
   - spotlight_images_count - общее количество снимков в периоде
...
1. Геопространственных данных в формате GPKG, которые могут быть загружены в ГИС-системы для визуального анализа:
